...
import { ArrowLeft, Sparkles, Trash2, Search, ChevronDown, Layers3, Clock3, ShieldCheck, CheckCircle2, FolderOpen, ClipboardCopy, ClipboardPlus, Check, HardDriveDownload } from "lucide-react";
...
export default function TestCases() {
    ...
    const fetchCases = useCallback(
        async (showError = true) => {
            try {
                const res = await featureTestCaseService.getTestCases(featureId);
                const normalized = normalizeVersions(res.data || []);
    ...
    const handleGenerate = async (showToast = true) => {
        try {
            ...
            const latestData = await fetchCases(false);
            ...
            if (showToast) {
                toast.success(versions.length === 0 ? "First version generated" : "New version generated");
            }
        } catch {
                        ...
                        <>
                            ...
                            <div className="space-y-3 pb-28">
                                {filteredCases.map((tc) => {
                                    const open = openCards[tc.testcaseId];
                                    return (
                                        <div
                                            key={tc.testcaseId}
                                        ...
                                        >
                                            <button
                                                onClick={() => toggleCard(tc.testcaseId)}
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center gap-2 shrink-0">
                                                    <button
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            copyToClipboard(formatTestcase(tc),
                                                                tc.testcaseId,
                                                                setCopiedId
                                                            );
                                                        }}
